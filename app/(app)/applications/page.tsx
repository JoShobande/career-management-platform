...
export default function ApplicationsPage() {
  const {applications, status, addApplication} = useApplications()
        ...
        <form className="space-y-4" onSubmit={onSubmit}>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div className="space-y-1">
              ...
              <select
                value={appStatus}
                onChange={(e) => setAppStatus(e.target.value as JobApplicationStatus)}
                className="w-full rounded-md border px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-gray-200"
              >
        ...
        {applications.length === 0 ? (
          <div className="rounded-lg border border-dashed bg-white p-8 text-center">
            ...
            <p className="mt-1 text-sm text-gray-500">
              Click <span className="font-medium">Add application</span> to log your first one.
            </p>
          </div>
        ) : (
          <div className="space-y-4">
            {applications.map((application) => (
              <div key={application.id} className="rounded-lg border bg-white p-4 shadow-sm hover:shadow transition">
